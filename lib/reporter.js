...
class AllureReporter extends events.EventEmitter {
    constructor (baseReporter, config, options = {}) {
        ...
        this.on('suite:start', (suite) => {
            const allure = this.getAllure(suite.specs)
        ...
        this.on('suite:end', (suite) => {
            this.getAllure(suite.specs).endSuite()
        })

        this.on('test:start', (test) => {
            const allure = this.getAllure(test.specs)
        ...
        this.on('test:pass', (test) => {
            this.getAllure(test.specs).endCase('passed')
        })
        ...
        this.on('runner:command', (command) => {
            const allure = this.getAllure(command.specs)
