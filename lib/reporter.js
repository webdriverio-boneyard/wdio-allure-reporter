...
class AllureReporter extends events.EventEmitter {
    ...
    startStep (stepName, timestamp) {
        ...
        this.openSteps++
        console.log(`${new Array(this.openSteps).join('\t')}OPEN: ${stepName.substr(0, 100)}`)
    }

    endStep (status, timestamp) {
        console.log(`${new Array(this.openSteps).join('\t')}CLOSE: ${this.getCurrentStep().name.substr(0, 100)}`)
