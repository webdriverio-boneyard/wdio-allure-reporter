...
class Instance {
    ...
    screenshot (screenshot) {
        let filename = this._getScreenshotFilename(screenshot)
        let data = new Buffer(screenshot.data, 'base64')
        let time = this._getTime()
    ...
    _endCase (type, force) {
        ...
        let status = test.result.status || 'broken'
        let err = test.result.err
        let time = test.result.time || this._getTime()
